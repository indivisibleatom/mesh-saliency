...
bool Vertex::isIsolated() const { return he == isolated.begin(); }
...
double Vertex::computeWeightedCurvature(VertexIter root,
...
bool Vertex::isPeakSaliency(VertexIter root,
                            const std::vector<double>& levelSaliencies) const {
  std::queue<VertexIter> queue;
  queue.push(root);
