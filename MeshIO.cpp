...
void MeshIO::preallocateMeshElements(const MeshData& data, Mesh& mesh) {
  ...
  int nB = std::max(
      0,
      2 - (int)chi);  // conservative approximation of number of boundary cycles
...
void MeshIO::checkNonManifoldVertices(const Mesh& mesh) {
  ...
  for (VertexCIter v = mesh.vertices.begin(); v != mesh.vertices.end(); v++) {
    ...
    if (vertexFaceMap[stringRep(v->position)] != valence) {
      std::cerr << "Warning: vertex " << v->index << " is nonmanifold."
                << std::endl;
    }
  }
}
...
bool MeshIO::buildMesh(const MeshData& data, Mesh& mesh) {
  ...
  for (unsigned int i = 0; i < data.normals.size(); i++) {
    VectorIter normal =
        mesh.normals.insert(mesh.normals.end(), Eigen::Vector3d());
  ...
  for (std::vector<std::vector<Index>>::const_iterator f = data.indices.begin();
    ...
    for (int i = 0; i < n; i++) {
      ...
      int uv = (*f)[i].uv;
      if (uv >= 0)
        halfEdges[i]->uv = data.uvs[uv];
      else
        halfEdges[i]->uv.setZero();

      int normal = (*f)[i].normal;
      if (normal >= 0)
        halfEdges[i]->normal = data.normals[normal];
      else
        halfEdges[i]->normal.setZero();

      halfEdges[i]->onBoundary = false;

      // keep track of which halfedges have flip edges defined (for deteting
      // boundaries)
      ...
      // if an edge between a and b has been created in the past, it is the flip
      // edge of the current halfedge
      if (a > b) std::swap(a, b);
      if (existingHalfEdges.find(std::pair<int, int>(a, b)) !=
          existingHalfEdges.end()) {
      ...
      if (edgeCount[std::pair<int, int>(a, b)] > 2) {
        std::cerr << "Error: edge " << a << ", " << b << " is non manifold"
                  << std::endl;
        return false;
      }
    }
  ...
  for (HalfEdgeIter currHe = mesh.halfEdges.begin();
       currHe != mesh.halfEdges.end(); currHe++) {
    // if a halfedge with no flip edge is found, create a new face and link it
    ...
    if (!hasFlipEdge[currHe]) {
      ...
      do {
        ...
        HalfEdgeIter newHe =
            mesh.halfEdges.insert(mesh.halfEdges.end(), HalfEdge());
  ...
  for (FaceCIter f = mesh.faces.begin(); f != mesh.faces.end(); f++) {
    ...
    do {
      out << vertexMap[stringRep(he->vertex->position)] << "/"
          << uvMap[stringRep(he->uv)] << "/" << normalMap[stringRep(he->normal)]
          << " ";
      j++;
